...
namespace Abp.Runtime.Caching.Redis {
    ...
    public class AbpRedisCache : CacheBase {
        ...
        public AbpRedisCache(
            ...
            : base(name, iocManager) {
            _database = redisCacheDatabaseProvider.GetDatabase();
            _serializer = redisCacheSerializer;
            KeyNormalizer = keyNormalizer;
            MultiTenancyConfig = multiTenancyConfig;
            Logger = iocManager.Resolve<ILoggerFactory>().CreateLogger(typeof(AbpRedisCache));
        }
        ...
        public override void Set(string key, object value, TimeSpan? slidingExpireTime = null, DateTimeOffset? absoluteExpireTime = null) {
            ...
            if (absoluteExpireTime.HasValue) {
                if (!_database.StringSet(redisKey, redisValue)) {
                    Logger.LogError("Unable to set key:{0} value:{1} in Redis", redisKey, redisValue);
                }
                else if (!_database.KeyExpire(redisKey, absoluteExpireTime.Value.UtcDateTime)) {
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", redisKey, absoluteExpireTime.Value.UtcDateTime);
                }
            }
            else if (slidingExpireTime.HasValue) {
                if (!_database.StringSet(redisKey, redisValue, slidingExpireTime.Value)) {
                    Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} in Redis", redisKey, redisValue, slidingExpireTime.Value);
                }
            }
                ...
                if (!_database.StringSet(redisKey, redisValue)) {
                    ...
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", redisKey, DefaultAbsoluteExpireTimeFactory(key).UtcDateTime);
                }
            }
                ...
                if (!_database.StringSet(redisKey, redisValue)) {
                    ...
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", redisKey, DefaultAbsoluteExpireTime.Value.UtcDateTime);
                }
            }
            else {
                if (!_database.StringSet(redisKey, redisValue, DefaultSlidingExpireTime)) {
                    Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} in Redis", redisKey, redisValue, DefaultSlidingExpireTime);
                }
            }
        }
        ...
        public override async Task SetAsync(string key, object value, TimeSpan? slidingExpireTime = null, DateTimeOffset? absoluteExpireTime = null) {
            ...
            if (absoluteExpireTime.HasValue) {
                if (!await _database.StringSetAsync(redisKey, redisValue)) {
                    Logger.LogError("Unable to set key:{0} value:{1} asynchronously in Redis", redisKey, redisValue);
                }
                else if (!await _database.KeyExpireAsync(redisKey, absoluteExpireTime.Value.UtcDateTime)) {
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", redisKey, absoluteExpireTime.Value.UtcDateTime);
                }
            }
            else if (slidingExpireTime.HasValue) {
                if (!await _database.StringSetAsync(redisKey, redisValue, slidingExpireTime.Value)) {
                    Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} asynchronously in Redis", redisKey, redisValue, slidingExpireTime.Value);
                }
            }
            else if (DefaultAbsoluteExpireTimeFactory != null) {
                if (!await _database.StringSetAsync(redisKey, redisValue)) {
                    ...
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", redisKey, DefaultAbsoluteExpireTimeFactory(key).UtcDateTime);
                }
            }
            else if (DefaultAbsoluteExpireTime.HasValue) {
                if (!await _database.StringSetAsync(redisKey, redisValue)) {
                    ...
                    Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", redisKey, DefaultAbsoluteExpireTime.Value.UtcDateTime);
                }
            }
            else {
                if (!await _database.StringSetAsync(redisKey, redisValue, DefaultSlidingExpireTime)) {
                    Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} asynchronously in Redis", redisKey, redisValue, DefaultSlidingExpireTime);
                }
            }
        }
        ...
        public override void Set(KeyValuePair<string, object>[] pairs, TimeSpan? slidingExpireTime = null, DateTimeOffset? absoluteExpireTime = null) {
            ...
            if (!_database.StringSet(redisPairs.ToArray())) {
                foreach (var pair in redisPairs) {
                    Logger.LogError("Unable to set key:{0} value:{1} in Redis", pair.Key, pair.Value);
                }
            ...
            if (absoluteExpireTime.HasValue) {
                foreach (var pair in redisPairs) {
                    if (!_database.KeyExpire(pair.Key, absoluteExpireTime.Value.UtcDateTime)) {
                        Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", pair.Key, absoluteExpireTime.Value.UtcDateTime);
                    }
                }
            }
            else if (slidingExpireTime.HasValue) {
                foreach (var pair in redisPairs) {
                    if (!_database.KeyExpire(pair.Key, slidingExpireTime.Value)) {
                        Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} in Redis", pair.Key, pair.Value, slidingExpireTime.Value);
                    }
                }
            }
            else if (DefaultAbsoluteExpireTimeFactory != null) {
                foreach (var pair in redisPairs) {
                    if (!_database.KeyExpire(pair.Key, DefaultAbsoluteExpireTimeFactory(pair.Key).UtcDateTime)) {
                        Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", pair.Key, DefaultAbsoluteExpireTimeFactory(pair.Key).UtcDateTime);
                    }
                }
            }
            else if (DefaultAbsoluteExpireTime.HasValue) {
                foreach (var pair in redisPairs) {
                    if (!_database.KeyExpire(pair.Key, DefaultAbsoluteExpireTime.Value.UtcDateTime)) {
                        Logger.LogError("Unable to set key:{0} to expire at {1:O} in Redis", pair.Key, DefaultAbsoluteExpireTime.Value.UtcDateTime);
                    }
                }
            }
            else {
                foreach (var pair in redisPairs) {
                    if (!_database.KeyExpire(pair.Key, DefaultSlidingExpireTime)) {
                        Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} in Redis", pair.Key, pair.Value, DefaultSlidingExpireTime);
                    }
                }
            }
        }
        ...
        public override async Task SetAsync(KeyValuePair<string, object>[] pairs, TimeSpan? slidingExpireTime = null, DateTimeOffset? absoluteExpireTime = null) {
            ...
            if (!await _database.StringSetAsync(redisPairs.ToArray())) {
                foreach (var pair in redisPairs) {
                    Logger.LogError("Unable to set key:{0} value:{1} asynchronously in Redis", pair.Key, pair.Value);
                }
            }
            else {
                if (absoluteExpireTime.HasValue) {
                    foreach (var pair in redisPairs) {
                        if (!await _database.KeyExpireAsync(pair.Key, absoluteExpireTime.Value.UtcDateTime)) {
                            Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", pair.Key, absoluteExpireTime.Value.UtcDateTime);
                        }
                    }
                }
                else if (slidingExpireTime.HasValue) {
                    foreach (var pair in redisPairs) {
                        if (!await _database.KeyExpireAsync(pair.Key, slidingExpireTime.Value)) {
                            Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} asynchronously in Redis", pair.Key, pair.Value, slidingExpireTime.Value);
                        }
                    }
                }
                else if (DefaultAbsoluteExpireTimeFactory != null) {
                    foreach (var pair in redisPairs) {
                        if (!await _database.KeyExpireAsync(pair.Key, DefaultAbsoluteExpireTimeFactory(pair.Key).UtcDateTime)) {
                            Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", pair.Key, DefaultAbsoluteExpireTimeFactory(pair.Key).UtcDateTime);
                        }
                    }
                }
                else if (DefaultAbsoluteExpireTime.HasValue) {
                    foreach (var pair in redisPairs) {
                        if (!await _database.KeyExpireAsync(pair.Key, DefaultAbsoluteExpireTime.Value.UtcDateTime)) {
                            Logger.LogError("Unable to set key:{0} to expire at {1:O} asynchronously in Redis", pair.Key, DefaultAbsoluteExpireTime.Value.UtcDateTime);
                        }
                    }
                }
                else {
                    foreach (var pair in redisPairs) {
                        if (!await _database.KeyExpireAsync(pair.Key, DefaultSlidingExpireTime)) {
                            Logger.LogError("Unable to set key:{0} value:{1} to expire after {2:c} asynchronously in Redis", pair.Key, pair.Value, DefaultSlidingExpireTime);
                        }
                    }
                }
            }
