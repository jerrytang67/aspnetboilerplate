# Script to replace Castle logging method calls with Microsoft logging

$files = Get-ChildItem -Path ".\src",".\test" -Include *.cs -Recurse | Where-Object {
    $content = Get-Content $_.FullName -Raw
    ($content -match 'using Microsoft\.Extensions\.Logging;') -and
    ($content -match '\.(Info|Warn|Debug|Error|Fatal|InfoFormat|WarnFormat|DebugFormat|ErrorFormat|FatalFormat)\(')
}
...
foreach ($file in $files) {
    ...
    # Simple method calls (no Format suffix)
    ...
    # Format method calls - these need special handling
    # Pattern: .InfoFormat("format {0}", arg) -> .LogInformation("format {Arg}", arg)
    ...
    # Replace ILoggerFactory.Create() with CreateLogger()
    $content = $content -replace '\.Create\(', '.CreateLogger('
